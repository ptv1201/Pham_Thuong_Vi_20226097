...
public class Aims {
	...
	public static void doEditMenu(Cart cart,Store store) {
		...
		if (scanner.nextInt() == 1) { // chọn số 1 trong editMenu để thêm vào store
			...
			doShowMenu(cart,store);// chọn 0 để quay lại showMenu
		scanner.close();
		doShowMenu(cart,store);
	}
	...
	public static void doCartMenu(Cart cart,Store store) {
		...
		if (scanner.nextInt() == 1) {// 1-Filter medias in cart
			filterMenu();
			if (scanner.nextInt() == 1) { // lọc theo tiêu đề, sau đó theo id
				Collections.sort(cart.itemsOrdered, Media.COMPARE_BY_TITLE_ID);
				System.out.println("filter by Title then Id:");
				for (Media media : cart.itemsOrdered) {
					System.out.println(media);
				}
			} else if (scanner.nextInt() == 2) {
				// lọc theo id, sau đó theo tiêu đề
				Collections.sort(cart.itemsOrdered, Media.COMPARE_BY_ID_TITLE);
			...
			doShowMenu(cart,store); // chọn 0 quay lại showMenu
